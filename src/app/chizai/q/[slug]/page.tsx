...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return pageMetadata({
    path: `/chizai/q/${slug}/`,
    title: questionPageTitle(q.title, "知的財産管理技能検定3級"),
...
export default async function ChizaiQuestionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = [
    quizJsonLd({ q, examName: "知的財産管理技能検定3級", path: `/chizai/q/${slug}/` }),
    breadcrumbJsonLd([
      { name: "ホーム", path: "/" },
      { name: "知的財産管理技能検定3級", path: "/chizai/" },
      ...(fieldSlug ? [{ name: q.field, path: `/chizai/field/${fieldSlug}/` }] : []),
      { name: `問${fieldIndex}`, path: `/chizai/q/${slug}/` },
    ]),
  ];
  ...
  return (
    <article className="theme-chizai pb-16">
      ...
      <AnswerReveal nextHref={nextQ ? `/chizai/q/${nextQ.slug}/` : undefined} nextLabel={nextQ ? `次の問題（問${idx + 2}）へ` : undefined} choices={q.choices} correctAnswer={q.correctAnswer} explanationHtml={q.content} exam="chizai" questionSlug={slug} courseAd={<ChizaiCourseAd />} />
