import { getChizai2QuestionsByField } from "@/lib/chizai2-questions";
...
const fieldMap: Record<string, { name: string; desc: string }> = {
  ...
  "copyright": { name: "著作権法", desc: "著作物・著作者の権利、保護期間、著作権の制限、著作隣接権、契約と利用許諾など" },
  "design": { name: "意匠法", desc: "意匠の定義、登録要件、意匠権の範囲と存続期間、関連意匠・組物の意匠など" },
  "trademark": { name: "商標法", desc: "商標の定義、登録要件、商標権の存続期間と更新、不使用取消審判、防護標章など" },
  "unfair": { name: "不正競争防止法", desc: "不正競争の類型、営業秘密の保護、形態模倣、限定提供データ、救済措置など" },
  "related": { name: "関連法規", desc: "独占禁止法、ライセンス契約と独禁法、種苗法、民法・契約実務との関係など" },
  "utility": { name: "実用新案法・種苗法", desc: "実用新案登録、技術評価書、権利行使の留意点、品種登録、育成者権など" },
  "treaty": { name: "国際条約", desc: "パリ条約、PCT、マドリッド協定議定書、ハーグ協定、TRIPS協定、ベルヌ条約など" },
  "practice": { name: "知財実務", desc: "ライセンス契約、職務発明、知財デューデリジェンス、出願・権利化戦略、知財管理など" },
};
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return pageMetadata({
    path: `/chizai2/field/${slug}/`,
    title: `知的財産管理技能検定2級｜${field.name} 練習問題`,
    description: `知的財産管理技能検定2級試験対策。${field.name}分野のオリジナル練習問題と詳細解説。各問に根拠条文を含む解説付きで効率的に学習できます。`,
  });
}

function QuestionCard({ q, index }: { q: Chizai2QuestionData; index: number }) {
  const diffColor = { A: "bg-green-100 text-green-800", B: "bg-amber-100 text-amber-800", C: "bg-red-100 text-red-800" }[q.difficulty];
  return (
    <a
      href={`/chizai2/q/${q.slug}/`}
...
export default async function Chizai2FieldPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const field = fieldMap[slug];
  if (!field) notFound();

  const questions = await getChizai2QuestionsByField(field.name);
  const diffCounts = {
