...
import {
  getEcoMoshi1,
  ECO_MOSHI_PASS_COUNT,
  ECO_MOSHI_PASS_POINTS,
  ECO_MOSHI_TIME_LIMIT_MIN,
} from "@/lib/eco-moshi";
import MoshiExam, { type MoshiQuestion } from "@/components/MoshiExam";

export const metadata: Metadata = pageMetadata({
  path: "/eco/moshi/",
  title: "eco検定 模擬試験 第1回（無料）｜四肢択一50問・90分・70点判定",
  description:
    "eco検定（環境社会検定試験）の無料模擬試験。本試験と同じ90分・100点満点70点以上の基準で四肢択一50問を受験でき、終了後に合格判定・分野別の弱点分析・全問の詳しい解説を確認できます。登録不要。",
});

export default async function EcoMoshiPage() {
  const takuitsu = await getEcoMoshi1();
  ...
  return (
    <div className="theme-eco pb-16">
      ...
      <p className="text-sm text-[color:var(--c-text)] leading-relaxed mb-5 max-w-2xl">
        ...
        eco検定はIBT・CBT方式のため過去問が公開されず、本番と同じ時間・同じ合否基準で通しで解ける場がありません。
        全員が同じ問題を同じ順序で解く固定問題なので、本番前の実力測定にそのまま使えます。
        出題は10分野からその問題数比に応じて選び、分野が連続しない順に並べています。
        eco検定は範囲の広さそのものが難所なので、頭を切り替えながら解く感覚をここで掴んでください。
      </p>
      ...
      <div className="card p-5 mb-6 text-sm text-[color:var(--c-text-sub)] space-y-2 max-w-2xl">
        ...
        <p><span className="font-bold text-[color:var(--c-ink)]">制限時間</span>　{ECO_MOSHI_TIME_LIMIT_MIN}分（自動採点）＝本試験と同じ</p>
        <p><span className="font-bold text-[color:var(--c-ink)]">合格基準</span>　{ECO_MOSHI_PASS_POINTS}点以上（{ECO_MOSHI_PASS_COUNT}問正解）＝本試験と同じ</p>
      ...
      <MoshiExam
        exam="eco"
        round={1}
        sessionKey="shikakumon-eco-moshi1-v1"
        questions={questions}
        timeLimitMin={ECO_MOSHI_TIME_LIMIT_MIN}
        passCount={ECO_MOSHI_PASS_COUNT}
        passLabel="70点／100点満点 以上"
        choiceLabel="四肢択一"
        questionPathPrefix="/eco/q/"
        topPath="/eco/"
        passPoints={ECO_MOSHI_PASS_POINTS}
      />
      ...
      <p className="text-xs text-[color:var(--c-text-sub)] mt-8 max-w-2xl leading-relaxed">
        ...
        時間を計らずに力試しをしたい方は
        <a href="/eco/mock/" className="underline hover:no-underline">本番形式テスト（ランダム20問）</a>へ、
        1問ずつじっくり学びたい方は<a href="/eco/" className="underline hover:no-underline">練習問題200問</a>へ。
      </p>
    </div>
  );
}
