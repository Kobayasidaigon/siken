import { getAllIsecSlugs, getIsecQuestion, getIsecQuestionsByField } from "@/lib/isec-questions";
...
import { ISEC_FIELD_SLUG_BY_NAME } from "@/lib/isec-fields";

const EXAM_NAME = "情報・サイバーセキュリティ管理士";

export async function generateStaticParams() {
  return getAllIsecSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const q = await getIsecQuestion(slug);
  if (!q) return {};
  return pageMetadata({
    path: `/isec/q/${slug}/`,
...
export default async function IsecQuestionPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const q = await getIsecQuestion(slug);
  if (!q) notFound();

  const fieldQuestions = await getIsecQuestionsByField(q.field);
  ...
  const fieldSlug = ISEC_FIELD_SLUG_BY_NAME[q.field] || "";
  ...
  const jsonLd = [
    quizJsonLd({ q, examName: EXAM_NAME, path: `/isec/q/${slug}/` }),
    breadcrumbJsonLd([
      { name: "ホーム", path: "/" },
      { name: EXAM_NAME, path: "/isec/" },
      ...(fieldSlug ? [{ name: q.field, path: `/isec/field/${fieldSlug}/` }] : []),
      { name: `問${fieldIndex}`, path: `/isec/q/${slug}/` },
    ]),
  ];
  ...
  return (
    <article className="theme-pii pb-16">
      ...
      <nav className="breadcrumb text-xs text-[color:var(--c-text-sub)] mb-4 flex flex-wrap gap-1">
        <a href="/">ホーム</a>
        <span>/</span>
        <a href="/isec/">{EXAM_NAME}</a>
        <span>/</span>
        {fieldSlug && (
          <>
            <a href={`/isec/field/${fieldSlug}/`}>{q.field}</a>
            <span>/</span>
          </>
        )}
      ...
      <div className="flex flex-wrap gap-2 mb-6 items-center">
        ...
        <BookmarkButton exam="isec" questionSlug={slug} />
      </div>
      ...
      <AnswerReveal
        nextHref={nextQ ? `/isec/q/${nextQ.slug}/` : undefined}
