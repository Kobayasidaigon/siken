import { getAllKangyoSlugs, getKangyoQuestion, getKangyoQuestionsByField } from "@/lib/kangyo-questions";
import { notFound } from "next/navigation";
import type { Metadata } from "next";
import AnswerReveal from "@/app/q/[slug]/AnswerReveal";
import BookmarkButton from "@/app/q/[slug]/BookmarkButton";
import KangyoCourseAd from "@/components/KangyoCourseAd";
import JsonLd from "@/components/JsonLd";
import { pageMetadata } from "@/lib/page-metadata";
import { quizJsonLd, breadcrumbJsonLd, questionPageTitle, questionPageDescription } from "@/lib/quiz-jsonld";

const fieldSlugMap: Record<string, string> = {
  "区分所有法（専有部分・共用部分・敷地・管理者）": "kubun1",
  "区分所有法（集会・規約・義務違反・復旧建替え）": "kubun2",
  "マンション標準管理規約": "kiyaku",
  "管理委託契約と標準管理委託契約書": "itaku",
  "民法と関連法令": "minpou",
  "管理組合の会計と仕訳": "kaikei",
  "マンション管理適正化法": "tekiseika",
  "建物の構造と維持保全": "hozen",
  "建築設備": "setsubi",
  "建築関連法規とその他": "kenchiku",
};

export async function generateStaticParams() {
  return getAllKangyoSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const q = await getKangyoQuestion(slug);
  if (!q) return {};
  return pageMetadata({
    path: `/kangyo/q/${slug}/`,
    title: questionPageTitle(q.title, "管理業務主任者"),
    description: questionPageDescription(q.description, q.questionText),
  });
}

export default async function KangyoQuestionPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const q = await getKangyoQuestion(slug);
  if (!q) notFound();

  const fieldQuestions = await getKangyoQuestionsByField(q.field);
  const idx = fieldQuestions.findIndex((x) => x.slug === slug);
  const fieldIndex = idx + 1;
  const fieldTotal = fieldQuestions.length;
  const prevQ = idx > 0 ? fieldQuestions[idx - 1] : null;
  const nextQ = idx < fieldQuestions.length - 1 ? fieldQuestions[idx + 1] : null;
  const fieldSlug = fieldSlugMap[q.field] || "";

  const difficultyLabel = { A: "易しい", B: "標準", C: "難しい" }[q.difficulty];
  const difficultyColor = { A: "bg-green-100 text-green-800", B: "bg-amber-100 text-amber-800", C: "bg-red-100 text-red-800" }[q.difficulty];

  const jsonLd = [
    quizJsonLd({ q, examName: "管理業務主任者", path: `/kangyo/q/${slug}/` }),
    breadcrumbJsonLd([
      { name: "ホーム", path: "/" },
      { name: "管理業務主任者", path: "/kangyo/" },
      ...(fieldSlug ? [{ name: q.field, path: `/kangyo/field/${fieldSlug}/` }] : []),
      { name: `問${fieldIndex}`, path: `/kangyo/q/${slug}/` },
    ]),
  ];

  return (
    <article className="theme-kashikin pb-16">
      <JsonLd data={jsonLd} />

      <nav className="breadcrumb text-xs text-[color:var(--c-text-sub)] mb-4 flex flex-wrap gap-1">
        <a href="/">ホーム</a><span>/</span>
        <a href="/kangyo/">管理業務主任者</a><span>/</span>
        {fieldSlug && <><a href={`/kangyo/field/${fieldSlug}/`}>{q.field}</a><span>/</span></>}
        <span className="text-[color:var(--c-ink)]">問{fieldIndex}</span>
      </nav>

      <h1 className="text-xl sm:text-2xl font-bold text-[color:var(--c-ink)] mb-3 leading-tight font-serif">{q.title}</h1>
      <div className="w-12 h-1 mb-4" style={{ background: "var(--c-kashikin)" }}></div>

      <div className="flex flex-wrap gap-2 mb-6 items-center">
        <span className="text-xs px-2 py-1 rounded-full font-medium" style={{ background: "var(--c-kashikin-soft)", color: "var(--c-kashikin-ink)" }}>
          {q.field} 問{fieldIndex}/{fieldTotal}
        </span>
        <span className={`text-xs px-2 py-1 rounded-full font-medium ${difficultyColor}`}>難易度{q.difficulty}（{difficultyLabel}）</span>
        <BookmarkButton exam="kangyo" questionSlug={slug} />
      </div>

      <section className="card p-5 mb-6" style={{ borderLeft: "4px solid var(--c-kashikin)" }}>
        <h2 className="text-sm font-bold mb-3" style={{ color: "var(--c-kashikin-ink)" }}>問題文</h2>
        <p className="text-sm text-[color:var(--c-text)] leading-relaxed whitespace-pre-wrap">{q.questionText}</p>
      </section>

      <AnswerReveal nextHref={nextQ ? `/kangyo/q/${nextQ.slug}/` : undefined} nextLabel={nextQ ? `次の問題（問${idx + 2}）へ` : undefined} choices={q.choices} correctAnswer={q.correctAnswer} explanationHtml={q.content} exam="kangyo" questionSlug={slug} courseAd={<KangyoCourseAd />} />

      <nav className="mt-8 flex justify-between items-center pt-4 border-t border-[color:var(--c-border)]">
        {prevQ ? (
          <a href={`/kangyo/q/${prevQ.slug}/`} className="text-sm text-[color:var(--c-text-sub)] no-underline hover:text-[color:var(--c-kashikin)]">← 問{idx}</a>
        ) : <span />}
        {fieldSlug && (
          <a href={`/kangyo/field/${fieldSlug}/`} className="text-sm text-[color:var(--c-text-sub)] no-underline hover:text-[color:var(--c-kashikin)]">
            {q.field}の一覧
          </a>
        )}
        {nextQ ? (
          <a href={`/kangyo/q/${nextQ.slug}/`} className="text-sm text-[color:var(--c-text-sub)] no-underline hover:text-[color:var(--c-kashikin)]">問{idx + 2} →</a>
        ) : <span />}
      </nav>
    </article>
  );
}
