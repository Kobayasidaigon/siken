/**
 * ビジネス実務法務検定2級 公式講座（全日本情報学習振興協会 SMART合格講座）の広告
 * 配置: ビジ法2級関連コラム記事の本文末尾、トップページ、問題・分野ページ
 *
...
export default function Bijihou2CourseAd({ headline, body }: Props = {}) {
  const finalHeadline = headline ?? "独学に不安があれば";
  const finalBody = body ?? "ビジネス実務法務検定2級の対策講座として、全日本情報学習振興協会のSMART合格講座があります。3級より範囲が広く条文の要件まで問われるため、体系的に学びたい方は検討してみてください。";
  const target = EXAM_AFFILIATE["bijihou2"];
  ...
  return (
    <aside className="theme-kashikin my-10 p-5 rounded-lg border border-[color:var(--c-border)] bg-[color:var(--c-bg-alt)]">
      ...
      <div className="flex flex-col items-start gap-3">
        <FreeLeadCTA exam="bijihou2" placement="course_ad" withBadge />
