 ...
 *
 * 【2026-09-22 追記】模試の結果画面に限ってメール登録を始めた (lib/moshi-reminder.ts)。
 * 上の「なぜメールを集めないか」で挙げた 4 点 (表示義務・同意の記録・配信停止・保存基盤) は
 * 姉妹サービス Studio 側の API と DB で満たし、このサイトには何も保存しない。
 * カレンダー登録はそのまま残す (メールを渡したくない人の選択肢として)。
 *
