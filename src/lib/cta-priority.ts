...
export const EXAM_SCHEDULES: Record<ExamSlug, UpcomingExam[]> = {
  ...
  itpass: [],
  // 情報・サイバーセキュリティ管理士は年4回実施だが、令和8年度の回次・申込期間を
  //   本環境から協会サイト(joho-gakushu.or.jp)に到達できず確認できなかった。
  //   推測の日付を入れるとカウントダウンが誤った締切を出すので、確認できるまで空にしておく。
  //   空の間はカウントダウンが出ず、答え合わせ直後CTAは Studio 優先になる(安全側)。
  //   TODO(isec): 協会の令和8年度試験日程で試験日・申込開始日・申込締切を確認して
  //     exam-dates.ts に ISEC_EXAMS を追加し、ここを差し替える。
  isec: [],
